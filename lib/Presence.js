...
module.exports = class Presence extends EventEmitter {
  ...
  enter (cb) {
    ...
    const listener = (data) => {
      if (!_.isObject(data) || !data.clientId)
        return

      if (data.clientId != this.client.connection.clientId)
        return

      if (data.action != 0)
        return

      if (this.client.connection.connectionId &&
        data.connectionId != this.client.connection.connectionId)
        return
